...
# 假设Target和Ammunition类定义已存在
class Target:
    def __init__(self, id: int, value: float, components: List[Tuple[float, float]]):
        self.id = id
        self.value = value
        self.components = components  # (权重, 易损性)

    def calculate_damage_effect(self, damage_prob: List[float]) -> float:
        total = 0.0
        for i, (weight, _) in enumerate(self.components):
            total += weight * damage_prob[i]
        return total
...
class MOPSO_Solver:
    def __init__(self, targets: List[Target], ammunitions: List[Ammunition],
        ...
        self.archive_size = 20  # 外部存档大小，与种群大小一致
        self.w = 0.4  # 惯性权重
        self.c1 = 1.5  # 个人认知系数
        self.c2 = 1.5  # 社会认知系数
        self.mutation_rate = 0.2  # 变异率，与NSGA-III一致

    def calculate_damage_efficiency(self, ammo_id: int, target_id: int) -> float:
        """与AHL_NSGA_II保持一致的毁伤效能计算"""
    ...
    def dominates(self, obj1: Tuple[float, float], obj2: Tuple[float, float]) -> bool:
        """与AHL_NSGA_II保持一致的支配关系判断"""
        return (obj1[0] >= obj2[0] and obj1[1] <= obj2[1]) and \
            (obj1[0] > obj2[0] or obj1[1] < obj2[1])

    def crowding_distance(self, archive: List[np.ndarray]) -> List[float]:
        """计算拥挤度，用于存档维护"""
        size = len(archive)
        if size == 0:
            return []
        ...
        distances = [0.0] * size
        objectives = [self.evaluate_objectives_with_penalty(ind)[:2] for ind in archive]

        for obj_idx in range(2):
            indexed_objs = list(zip(range(size), [obj[obj_idx] for obj in objectives]))

            if obj_idx == 0:  # 费效比 max
    ...
    def mutate(self, chromosome: np.ndarray) -> np.ndarray:
        """MOPSO中的变异操作，与NSGA-III的mutate类似"""
    ...
    def update_archive(self, archive: List[np.ndarray], candidates: List[np.ndarray]) -> List[np.ndarray]:
        """更新外部存档：添加非支配解，并使用拥挤度维护大小"""
        combined = archive + candidates
        # 移除支配的解
        to_keep = []
        objectives = [self.evaluate_objectives_with_penalty(ind)[:2] for ind in combined]
        for i in range(len(combined)):
            ...
            for j in range(len(combined)):
                if i != j and self.dominates(objectives[j], objectives[i]):
                    ...
                    break
            if not dominated:
                to_keep.append(combined[i])

        # 如果超过存档大小，使用拥挤度移除
        if len(to_keep) > self.archive_size:
            distances = self.crowding_distance(to_keep)
            sorted_indices = np.argsort(distances)  # 升序，移除最小拥挤度的
            to_keep = [to_keep[i] for i in sorted_indices[-self.archive_size:]]

        return to_keep

    def select_leader(self, archive: List[np.ndarray]) -> np.ndarray:
        """从存档中选择领导者（gbest），使用轮盘赌基于拥挤度逆"""
        if not archive:
            return None
        distances = self.crowding_distance(archive)
        # 逆拥挤度（小拥挤优先？  No, in MOPSO, prefer less crowded for diversity
        inv_distances = [1 / d if d > 0 else 1e10 for d in distances]  # 大inv表示拥挤小
        total = sum(inv_distances)
        pick = random.uniform(0, total)
        current = 0
        for i, inv_d in enumerate(inv_distances):
            current += inv_d
            if current > pick:
                return archive[i]
        return archive[-1]
    ...
    def solve(self) -> Tuple[List[Tuple[np.ndarray, Tuple[float, float], int, float]], int]:
        """MOPSO主流程"""
        positions = self.initialize_population()  # 粒子位置
        velocities = [np.zeros_like(pos) for pos in positions]  # 初始速度0
        pbests = [pos.copy() for pos in positions]  # 个人最佳
        pbest_objs = [self.evaluate_objectives_with_penalty(pos)[:2] for pos in positions]
        archive = []  # 外部存档
        all_feasible_with_metadata = []

        # 初始存档更新
        archive = self.update_archive(archive, positions)
        ...
        for generation in range(self.max_generations):
            ...
            for i in range(self.population_size):
                leader = self.select_leader(archive)
                if leader is None:
                    continue
                ...
                r1 = random.random()
                r2 = random.random()
                velocities[i] = self.w * velocities[i] + self.c1 * r1 * (pbests[i] - positions[i]) + self.c2 * r2 * (leader - positions[i])

                # 更新位置
                positions[i] = positions[i] + velocities[i].astype(int)  # 整数化
                positions[i] = np.maximum(positions[i], 0)  # 非负

                # 变异
                positions[i] = self.mutate(positions[i])

                # 修复
                positions[i] = self.repair_solution(positions[i])

                # 评估
                obj = self.evaluate_objectives_with_penalty(positions[i])[:2]

                # 更新pbest
                if self.dominates(obj, pbest_objs[i]):
            ...
            # 收敛检测（基于存档最佳费效比）
            if archive:
                current_best_ce = max([self.evaluate_objectives_with_penalty(sol)[0] for sol in archive])
...
def main():
    ...
    solver = MOPSO_Solver(targets, ammunitions, adaptability_matrix, 0.8)

    print("开始求解静态弹目匹配问题（MOPSO算法）...")
...
def run_multiple_tests(test_times: int = 15):
    ...
    print(f"开始 {test_times} 次MOPSO独立测试...")
    ...
    print(f"所有测试中的全局最大费效比: {global_max_ce:.4f}" if global_max_ce else "无有效解")
    ...
    # 输出全局最优解详情
    print("\n" + "=" * 60)
    print(f"MOPSO全局最优解（最高费效比: {global_best_ce:.4f}）")
    print("=" * 60)
