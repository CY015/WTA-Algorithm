...
class StaticWeaponTargetAssignment:
    ...
    def __init__(self, targets: List[Target], ammunitions: List[Ammunition], 
        ...
        # 普通交叉变异参数（固定值，不随代数变化）
        self.crossover_rate = 0.7  # 固定交叉概率
        self.mutation_rate = 0.1   # 固定变异概率
        self.local_search_rate = 0.3  # 固定局部搜索概率
    ...
    def solve(self) -> List[Tuple[np.ndarray, Tuple[float, float], int, float]]:
        ...
        for generation in range(self.max_generations):
            ...
            if generation % 20 == 0:
                ...
                print(f"  固定参数: Crossover={self.crossover_rate:.3f}, "
                      f"Mutation={self.mutation_rate:.3f}, LocalSearch={self.local_search_rate:.3f}")
    ...
    def heuristic_local_search(self, chromosome: np.ndarray) -> np.ndarray:
        ...
        for j in range(target_count):
            ...
            elif current_damage > self.damage_threshold + 0.1:
                # 严重过量毁伤：减少效费比最低的弹药
                non_zero_ammos = []
                for i in range(ammo_count):
                    if improved_chromosome[i, j] > 1:
    ...
    def simple_mutation(self, chromosome: np.ndarray) -> np.ndarray:
        ...
        # 对每个弹药-目标对进行变异（固定概率，仅扰动变异）
        for i in range(ammo_count):
            for j in range(target_count):
                if random.random() < self.mutation_rate:
                    # 仅使用扰动变异（无随机选择策略）
                    change = random.choice([-1, 1])  # 较小的扰动步长
