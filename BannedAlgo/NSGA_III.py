...
class NSGAIII_Solver:
    def __init__(self, targets: List[Target], ammunitions: List[Ammunition],
        ...
        self.population_size = 20  # 与AHL_NSGA_II保持一致
        self.max_generations = 500  # 与AHL_NSGA_II保持一致

        # 固定参数（非自适应）
        self.crossover_rate = 0.8  # 中间值
        self.mutation_rate = 0.2  # 中间值
    ...
    def solve(self) -> Tuple[List[Tuple[np.ndarray, Tuple[float, float], int, float]], int]:
        """与AHL_NSGA_II保持一致的主流程，仅修改交叉变异部分"""
        ...
        for generation in range(self.max_generations):
            ...
            # 合并种群+非支配排序+拥挤度选择（与AHL_NSGA_II完全一致）
...
def main():
    ...
    solver = NSGAIII_Solver(targets, ammunitions, adaptability_matrix, 0.8)

    print("开始求解静态弹目匹配问题（NSGA-III算法）...")
...
def run_multiple_tests(test_times: int = 15):
    ...
    print(f"开始 {test_times} 次NSGA-III独立测试...")
    for test_idx in range(test_times):
        ...
        feasible_ratio = 1.0  # NSGA-III修复机制确保解可行
    ...
    # 输出全局最优解详情
    print("\n" + "=" * 60)
    print(f"NSGA-III全局最优解（最高费效比: {global_best_ce:.4f}）")
    print("=" * 60)
